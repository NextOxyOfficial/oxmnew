...
class BankAccountSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = BankAccount
        fields = [
            'id', 'name', 'owner', 'owner_name', 'owner_username', 'balance', 
    ...
    def get_total_credits(self, obj):
        return obj.transactions.filter(type='credit', status='verified').aggregate(
            total=serializers.models.Sum('amount')
        )['total'] or 0

    def get_total_debits(self, obj):
        return obj.transactions.filter(type='debit', status='verified').aggregate(
            total=serializers.models.Sum('amount')
        )['total'] or 0
...
class TransactionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Transaction
        fields = [
            'id', 'account', 'type', 'amount', 'purpose', 'verified_by', 
            'status', 'date', 'updated_at', 'reference_number',
            'verified_by_details', 'account_name'
        ]
