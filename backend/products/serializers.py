...
class ProductVariantSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'color', 'size', 'weight', 'weight_unit', 'custom_variant',
            'buy_price', 'sell_price', 'stock', 'profit', 'profit_margin',
            'weight_display', 'created_at', 'updated_at'
        ]
...
class ProductCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = [
            'name', 'category', 'supplier', 'location', 'details',
            'hasVariants', 'buyPrice', 'sellPrice', 'colorSizeVariants', 'photos'
        ]
        extra_kwargs = {
    ...
    def validate(self, data):
        ...
        if has_variants:
            ...
            for i, variant_data in enumerate(variants_data):
                ...
                has_identifying_field = bool(
                    color or size or weight or custom_variant)
    ...
    def create(self, validated_data):
        ...
        # If not using variants, set stock to 0 for single product
        if not validated_data.get('has_variants', False):
            validated_data['stock'] = 0  # We'll handle stock separately
...
class ProductSaleSerializer(serializers.ModelSerializer):
    ...
    variant_display = serializers.CharField(
        source='variant.__str__', read_only=True)

    class Meta:
        model = ProductSale
        fields = [
            'id', 'product', 'product_name', 'variant', 'variant_display',
            'quantity', 'unit_price', 'total_amount', 'customer_name',
            'customer_phone', 'customer_email', 'notes', 'sale_date'
        ]
