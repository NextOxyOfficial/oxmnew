...
def dashboard_stats(request):
    ...
    # Order stats (if Order model exists)
    try:
        ...
        # Fallback if Order model doesn't exist
        order_count = 342
    ...
    recent_activities = [
        {
            ...
            'description': f'Order from customer - ${(total_revenue / order_count):.0f}' if order_count > 0 else 'Order from customer - $450',
...
def search_global(request):
    ...
    for customer in customers:
        results.append({
            ...
            'avatar': ''.join([word[0].upper() for word in customer.name.split()[:2]])
        })
