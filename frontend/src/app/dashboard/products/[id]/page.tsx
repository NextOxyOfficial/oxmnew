...
export default function ProductDetailsPage() {
  ...
  const loadStockHistory = async () => {
    ...
    try {
      ...
      const transformedHistory: StockEntry[] = movements.map((movement: any) => {
        console.log('Processing movement:', movement);
        
        // Determine if it's an add or remove operation
        const isAddition = movement.quantity > 0 || movement.movement_type === 'in';
        ...
        if (movement.user_name && movement.user_name.trim()) {
          ...
          if (typeof movement.user === 'string') {
            ...
            createdBy = `${movement.user.first_name || ''} ${movement.user.last_name || ''}`.trim();
        ...
        return {
          ...
          reason: movement.reason || movement.movement_type_display || "Stock adjustment",
              ...
              <div className="flex border-b border-slate-700/50 bg-slate-800/30">
                ...
                <button
                  onClick={() => {
                    ...
                    if (stockHistory.length === 0 && !isLoadingHistory && product?.id) {
                      loadStockHistory();
                    }
                  }}
                  className={`flex-1 py-3 px-4 text-sm font-medium transition-colors cursor-pointer ${
                  ...
                  <div className="space-y-4">
                    ...
                    {isLoadingHistory ? (
                      <div className="space-y-3">
                        {[1, 2, 3].map((i) => (
                          <div key={i} className="bg-slate-800/30 border border-slate-700/50 rounded-lg p-4 animate-pulse">
                      ...
                      <div className="bg-slate-800/30 border border-slate-700/50 rounded-lg p-8 text-center">
                        ...
                        <h4 className="text-slate-300 font-medium mb-2">No Stock History</h4>
                        <p className="text-slate-400 text-sm mb-4">
                          No stock movements have been recorded for this product yet.
                        </p>
                        <p className="text-slate-500 text-xs">
                          Stock movements will appear here when you add stock, make sales, or perform adjustments.
                        </p>
                      </div>
                    ) : (
                      <div className="space-y-3">
                          ...
                          >
                            <div className="flex items-start justify-between">
                              <div className="flex-1">                              <div className="flex items-center space-x-3 mb-2">
                                ...
                                {entry.movement_type_display && entry.movement_type_display !== entry.reason && (
